 ...
 */
export function compareRules(expected, actual, declared) {
  if (actual === null || typeof actual !== "object" || Array.isArray(actual)) return ["did not print a JSON object"];
  const problems = [];
  // The module's manifest says which facts it repeats; it must print exactly those, so a fact cannot
  // quietly stop being compared by being left out.
  for (const key of declared) if (!(key in actual)) problems.push(`does not state \`${key}\`, which its module.json says it repeats`);
  for (const key of Object.keys(actual)) {
    if (!(key in expected)) problems.push(`states \`${key}\`, which the rules do not have`);
...
function main() {
  ...
  for (const module of targets) {
    ...
    const problems = compareRules(rules, actual, module.facts.keys);
