...
5. Add its Dependabot entries, and its lines in `README.md`, `AGENTS.md` and wherever else it belongs, each inside markers. `check-hygiene` fails if its Dependabot entry is missing. A new task service says how to start it under `taskApi` in its `module.json`, and must pass every case in `scripts/contract/tasks-api.json`, its configuration cases, request ids and log lines included. A module that repeats a fact without serving it lists the facts under `facts` in its `module.json` and prints them from a `facts` script, which `scripts/check-facts.mjs` compares with the files that state them. Either way it owns `scripts/rules` in `features.json`, beside the others that do, and `scripts/contract` too if it serves or calls the task API.
